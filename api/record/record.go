...
func createRecord(r RecordRequest) (int, error) {
	// Insert record
	query := "INSERT INTO record (user_id, room_id, pot_id, time_interval, ingredient_id, interrupt, status) VALUES (?, ?, ?, 0, ?, ?, ?)"
	result, err := mariadb.DB.Exec(query, r.UserID, r.RoomID, r.PotID, r.IngredientID, r.Interrupt, r.Status)
	if err != nil {
